...
def train_epoch_inner(model, optimizer, batches):
    ...
    current_epoch = optimizer.step.value // batches.shape[0]
    
    def train_step(model_opt, batch):
        model, optimizer = model_opt
        (loss_, (artfcs_, stats)), grads = grad_loss_fn(model, batch, current_epoch)
