...
class Concatenate(AbstractBijection):
    ...
    def transform(self, x, condition=None):
        ...
        y_parts = [b.transform(x_part, condition) for b, x_part in zip(self.bijections, x_parts, strict=True)]
        return jnp.concatenate(y_parts, axis=self.axis)

    def transform_and_log_det(self, x, condition=None):
        x_parts = jnp.array_split(x, self.split_idxs, axis=self.axis)

        ys_log_dets = [b.transform_and_log_det(x, condition) for b, x in zip(self.bijections, x_parts, strict=True)]
    ...
    def inverse(self, y, condition=None):
        ...
        x_parts = [b.inverse(y_part, condition) for b, y_part in zip(self.bijections, y_parts, strict=True)]
        return jnp.concatenate(x_parts, axis=self.axis)

    def inverse_and_log_det(self, y, condition=None):
        y_parts = jnp.array_split(y, self.split_idxs, axis=self.axis)

        xs_log_dets = [b.inverse_and_log_det(y, condition) for b, y in zip(self.bijections, y_parts, strict=True)]
...
class Stack(AbstractBijection):
    ...
    def transform(self, x, condition=None):
        ...
        y_parts = [b.transform(x, condition) for (b, x) in zip(self.bijections, x_parts, strict=True)]
        return jnp.stack(y_parts, self.axis)

    def transform_and_log_det(self, x, condition=None):
        x_parts = self._split_and_squeeze(x)
        ys_log_det = [b.transform_and_log_det(x, condition) for b, x in zip(self.bijections, x_parts, strict=True)]
    ...
    def inverse(self, y, condition=None):
        ...
        x_parts = [b.inverse(y, condition) for (b, y) in zip(self.bijections, y_parts, strict=True)]
        return jnp.stack(x_parts, self.axis)

    def inverse_and_log_det(self, y, condition=None):
        y_parts = self._split_and_squeeze(y)
        xs_log_det = [b.inverse_and_log_det(y, condition) for b, y in zip(self.bijections, y_parts, strict=True)]
