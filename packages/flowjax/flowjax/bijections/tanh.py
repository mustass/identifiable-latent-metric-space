...
class LeakyTanh(AbstractBijection):
    ...
    def inverse(self, y, condition=None):
        ...
        true_op = jnp.where(is_linear, y, 0)
        false_op = jnp.where(is_linear, 0, y)

        x_linear = (true_op - jnp.sign(true_op) * self.intercept) / self.linear_grad
        x_arctan = jnp.arctanh(false_op)
