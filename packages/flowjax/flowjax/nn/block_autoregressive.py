...
class BlockAutoregressiveLinear(eqx.Module):
    ...
    def __init__(
        ...
        *w_key, bias_key, scale_key = random.split(key, n_blocks + 2)
        ...
        self.weights = init(w_key[0], (out_features, in_features)) * (self.b_tril_mask + self.b_diag_mask)
        self.bias = (random.uniform(bias_key, (out_features,)) - 0.5) * (2 / jnp.sqrt(out_features))
    ...
    def get_normalised_weights(self):
        ...
        weights = jnp.exp(self.weights) * self.b_diag_mask + self.weights * self.b_tril_mask
