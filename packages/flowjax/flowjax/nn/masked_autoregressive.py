...
class AutoregressiveMLP(Module):
    ...
    def __init__(
        ...
        *,
        ...
        in_ranks, hidden_ranks, out_ranks = (jnp.asarray(a, jnp.int32) for a in (in_ranks, hidden_ranks, out_ranks))
        masks = []
        if depth == 0:
            masks.append(rank_based_mask(in_ranks, out_ranks, eq=False))
        else:
            masks.append(rank_based_mask(in_ranks, hidden_ranks, eq=True))
            masks.extend(rank_based_mask(hidden_ranks, hidden_ranks, eq=True) for _ in range(depth - 1))
            masks.append(rank_based_mask(hidden_ranks, out_ranks, eq=False))

        keys = random.split(key, len(masks))
        layers = tuple(MaskedLinear(mask, key=key) for mask, key in zip(masks, keys, strict=True))
