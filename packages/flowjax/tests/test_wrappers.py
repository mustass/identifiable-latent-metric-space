...
def test_WeightNormalization():
    ...
    assert pytest.approx(expected) == jnp.linalg.norm(unwrap(weight_norm), axis=-1, keepdims=True)

    # Test under vmap
    arr = jr.normal(jr.PRNGKey(0), (5, 10, 3))
    weight_norm = eqx.filter_vmap(WeightNormalization)(arr)
    expected = unwrap(weight_norm.scale)
    assert pytest.approx(expected) == eqx.filter_vmap(lambda arr: jnp.linalg.norm(arr, axis=1, keepdims=True))(
        unwrap(weight_norm)
    )
