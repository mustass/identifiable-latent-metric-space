...
from ilms.data import get_arrays, files, files_celeba
...
def run_experiment(cfg: DictConfig, wandb_logger, test_dataset, pairs):
    ...
    for i, batch in enumerate(
        ...
        for i, pair in enumerate(batch):
            input.append(jnp.array([test_dataset[pair[0]], test_dataset[pair[1]]]))
    ...
    print(
        ...
        len(euclids_latent),
        len(euclids_ambient),
        len(euclids_reconstructed_ambient),
    )
...
def main(cfg: DictConfig):
    ...
    _, _, _, _, test_images, test_labels = get_arrays(
        cfg.inference["datamodule"]["dataset_root"],
        (
            files_celeba
            if Path(cfg.inference["datamodule"]["dataset_root"]).name == "celeba"
            else files
        ),
    )
    ...
    for experiment in experiments:
        ...
        output.append(config.model.params.num_decoders)
        outputs.append(output)

        file_path = os.path.join(
            cfg["general"]["checkpoint_path"],
            os.path.basename(os.path.normpath(experiment[0])),
        )
