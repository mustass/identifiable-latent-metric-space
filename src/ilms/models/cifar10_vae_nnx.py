# python scripts/train.py general.run_name=svhn_prc_test datamodule.batch_size=128 training.seed=0 model=svhn_vae_nnx datamodule=svhn general.project_name=icml_rebuttle_svhn training.num_epochs=500 training.checkpoint=/data/ilmsdata/checkpoints/test datamodule.dataset_root=/data/ilmsdata/svhn
...
class VAE(nnx.Module):
    ...
    class Decoder(nnx.Module):
        def __init__(self, opts, rngs):
            ...
            self.fc_dec = nnx.Sequential(
                nnx.Linear(opts.z_dim, 256 * 4, rngs=rngs), 
                nnx.elu
            )

            self.convs = nnx.Sequential(
                ResizeAndConv(
                    256, 64, kernel_size=(2, 2), strides=(1, 1), padding=1, rngs=rngs
                ),
                nnx.elu,
                ResizeAndConv(
                ...
                nnx.elu,
                ResizeAndConv(
                    8, 3, kernel_size=(3, 3), strides=(2, 2), padding=1, rngs=rngs
                ),
            )
    ...
    def __init__(self, opts={}, *, rngs: nnx.Rngs):
        ...
        self.encoder = nnx.Sequential(
            nnx.Conv(3, 16, kernel_size=(3, 3), strides=2, padding=1, rngs=rngs),
            nnx.elu,
            nnx.Conv(16, 32, kernel_size=(3, 3), strides=2, padding=1, rngs=rngs),
            nnx.elu,
            ...
            nnx.elu,
            nnx.Conv(128, 512, kernel_size=(3, 3), strides=2, padding=1, rngs=rngs),
            nnx.elu,
        )

        self.enc_mu = nnx.Linear(512, z_dim, rngs=rngs)
        self.enc_logvar = nnx.Linear(512, z_dim, rngs=rngs)
