...
class GatedConv2d(eqx.Module):
    conv: eqx.Module

    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1, key=None):
        self.conv = nn.Conv2d(in_channels, 2 * out_channels, kernel_size, stride, padding, dilation, key=key)

    def __call__(self, inputs):
        temps = self.conv(inputs)
        outputs = jnn.glu(temps, dim=1)
        return outputs


class GatedConvTranspose2d(eqx.Module):
    conv_transpose: eqx.Module

    def __init__(
        self, in_channels, out_channels, kernel_size, stride, padding, output_padding=0, dilation=1, key=None
    ):
        self.conv_transpose = nn.ConvTranspose2d(
            in_channels, 2 * out_channels, kernel_size, stride, padding, output_padding, dilation=dilation, key=key
        )
...
class SylvesterFlowConvEncoderNet(eqx.Module):
    context_features: int
    last_kernel_shape: tuple
    gated_conv_layers: list
    context_layer: eqx.Module

    def __init__(self, context_features, last_kernel_shape=(7, 7), key=None):
        self.context_features = context_features
        self.last_kernel_shape = last_kernel_shape

        keys = jax.random.split(key, 8)

        self.gated_conv_layers = [
            GatedConv2d(in_channels=1, out_channels=32, kernel_size=5, padding=2, stride=1, key=keys[0]),
            GatedConv2d(in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=2, key=keys[1]),  # 2
            GatedConv2d(in_channels=32, out_channels=64, kernel_size=5, padding=2, stride=1, key=keys[2]),  # 3
            GatedConv2d(in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=2, key=keys[3]),  # 4
            GatedConv2d(in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=1, key=keys[4]),  # 5
            GatedConv2d(in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=1, key=keys[5]),  # 6
            GatedConv2d(
                in_channels=64, out_channels=256, kernel_size=self.last_kernel_shape, padding=0, stride=1, key=keys[6]
            ),  # 7
        ]

        self.context_layer = nn.Linear(in_features=256, out_features=self.context_features, key=keys[7])
...
class SylvesterFlowConvDecoderNet(eqx.Module):
    latent_features: int
    last_kernel_shape: tuple
    gated_conv_transpose_layers: list

    def __init__(self, latent_features, last_kernel_shape=(7, 7), key=None):
        ...
        self.gated_conv_transpose_layers = [
            ...
            GatedConvTranspose2d(
                in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=1, key=keys[1]
            ),  # 2
            GatedConvTranspose2d(
                in_channels=64, out_channels=32, kernel_size=5, padding=2, stride=2, output_padding=1, key=keys[2]
            ),  # 3
            GatedConvTranspose2d(
                in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=1, key=keys[3]
            ),  # 4
            GatedConvTranspose2d(
                in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=2, output_padding=1, key=keys[4]
            ),  # 5
            GatedConv2d(in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=1, key=key[5]),  # 6
            GatedConv2d(in_channels=32, out_channels=1, kernel_size=1, padding=0, stride=1, key=key[6]),  # 7
        ]
...
class ModifiedConvEncoder(eqx.Module):
    ...
    def __init__(
        ...
        keys = jax.random.split(key, levels + 2)
        ...
        self.final_layer = nn.Linear(in_features=self.flat_dim, out_features=out_features, key=keys[-1])
    ...
    def __call__(self, inputs, context=None):
        ...
        outputs = (3 - 1e-4) * jnp.tanh(outputs)
        return outputs
