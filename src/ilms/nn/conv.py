...
class ConvEncoder(eqx.Module):
    ...
    def __init__(
        self, context_features, channels_multiplier, activation=jnn.relu, key=None
    ):
        ...
        self.initial_layer = nn.Conv2d(
            1, channels_multiplier, kernel_size=1, key=keys[0]
        )
        self.residual_blocks = [
            ResidualBlock(in_channels=channels_multiplier, key=keys[1]),
            ResidualBlock(
                in_channels=channels_multiplier, resample="down", key=keys[2]
            ),
            ResidualBlock(in_channels=channels_multiplier * 2, key=keys[3]),
            ResidualBlock(
                in_channels=channels_multiplier * 2, resample="down", key=keys[4]
            ),
            ResidualBlock(in_channels=channels_multiplier * 4, key=keys[5]),
            ResidualBlock(
                in_channels=channels_multiplier * 4, resample="down", key=keys[6]
            ),
        ]
    ...
    def __call__(self, inputs):
        ...
        outputs = self.final_layer(
            temps.reshape(-1, 4 * 4 * self.channels_multiplier * 8).squeeze()
        )
        return outputs
...
class ConvDecoder(eqx.Module):
    ...
    def __init__(
        self, latent_features, channels_multiplier, activation=jnn.relu, key=None
    ):
        ...
        self.residual_blocks = [
            ...
            ResidualBlock(
                in_channels=channels_multiplier * 4, resample="up", key=keys[4]
            ),
            ResidualBlock(in_channels=channels_multiplier * 2, key=keys[5]),
            ResidualBlock(
                in_channels=channels_multiplier * 2, resample="up", key=keys[6]
            ),
        ]
        self.final_layer = nn.Conv2d(
            in_channels=channels_multiplier, out_channels=1, kernel_size=1, key=keys[7]
        )
        self.mean_head = nn.Linear(
            in_features=(4 * 4 * channels_multiplier * 8), out_features=1, key=keys[8]
        )
        self.sigma_head = nn.Linear(
            in_features=(4 * 4 * channels_multiplier * 8), out_features=1, key=keys[9]
        )

    def __call__(self, inputs):
        temps = (
            self.initial_layer(inputs)
            .reshape(-1, self.channels_multiplier * 8, 4, 4)
            .squeeze()
        )
        for residual_block in self.residual_blocks:
            temps = residual_block(temps)
...
class ModifiedConvEncoder(eqx.Module):
    ...
    def __init__(
        ...
        self.initial_layer = nn.Conv2d(
            channels_in, channels_in * channels_multiplier, kernel_size=1, key=keys[0]
        )
        blocks = []
        for i in range(levels):
            ...
            blocks.append(
                ResidualBlock(
                    in_channels=channels_in * channels_multiplier * 2**i, key=key1
                )
            )
            blocks.append(
                ResidualBlock(
                    in_channels=channels_in * channels_multiplier * 2**i,
                    resample="down",
                    key=key2,
                )
            )
        ...
        self.mean_head = nn.Linear(
            in_features=self.flat_dim, out_features=out_features, key=keys[-2]
        )
        self.sigma_head = nn.Linear(
            in_features=self.flat_dim, out_features=out_features, key=keys[-1]
        )
...
class Conv2dSameSize(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, key=None):
        ...
        super().__init__(
            in_channels, out_channels, kernel_size, padding=same_padding, key=key
        )
