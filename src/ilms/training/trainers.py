...
class TrainerModule:
    def __init__(self, model: nnx.Module, config: DictConfig, wandb_logger):
        ...
        with open(os.path.join(self.model_checkpoint_path, "config.yml"), "w") as f:
    ...
    def train_model(self, train_array, val_array, num_epochs, logger=None):
        ...
        for epoch_idx in range(num_epochs):
            ...
            if epoch_idx % self.eval_every == 0:
                ...
                if (val_elbo - best_val_elbo) > self.early_stopping_grace:
                    ...
                    logging.info(
                        f"Saved checkpoint with val_elbo={val_elbo:.2f} at epoch {epoch_idx}."
                    )
                elif (not (val_elbo - best_val_elbo) > self.early_stopping_grace) and (
                    epoch_idx - best_val_elbo_epoch
                ) > self.early_stopping_patience:
                    logging.info(
                        f"Early stopping invoked after {epoch_idx-best_val_elbo_epoch} of no improvement above {self.early_stopping_grace}"
                    )
                    break
                else:
                    continue
        ...
        if os.path.exists(best_checkpoint_name):
            os.rename(
                best_checkpoint_name, f"{self.model_checkpoint_path}/best_model.pickle"
            )
        logging.info(
            f"Renamed the best model checkpoint to {self.model_checkpoint_path}/best_model.pickle"
        )
    ...
    def eval_model(self, val_array, step, prefix="val"):
        ...
        for dict_key, dict_val in stats.items():
            self.logger.log(
                {f"{prefix}_" + dict_key + "_epoch": dict_val.item()}, step=step
            )
